...
订阅按日历月或日历年计算周期，并在月底自动取目标月份最后一天。新写入的周期截止统一归一到“系统设置 → 通用 → 时区”中目标日期的 23:59:59，手工续期和 Stripe 同步使用同一规则。管理界面分别显示商业到期和宽限截止，可使用必填幂等键一次续期一个周期；提前续费从原到期日顺延，逾期续费从当天起算。版本化签名令牌把固定 7 天在线校验租约与商业到期、商业宽限明确分开。
